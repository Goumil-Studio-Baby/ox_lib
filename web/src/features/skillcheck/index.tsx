...
const useStyles = createStyles((theme, params: { difficultyOffset: number; isExiting: boolean; glass: ReturnType<typeof useGlassStyle>; disableAnimations?: boolean; config?: any }) => {
  // config.theme.skillCheckColor overrides the Mantine primary color
  const themeColor = params.config?.theme?.skillCheckColor
...
const SkillCheck: React.FC = () => {
  ...
  // config.theme.skillCheckColor overrides Mantine primary for particles
  const safeThemeColor = config.theme.skillCheckColor
    || theme.colors?.[theme.primaryColor]?.[theme.fn?.primaryShade() ?? 8]
    || '#ef4444';
