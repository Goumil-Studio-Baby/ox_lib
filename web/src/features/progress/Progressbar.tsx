...
const Progressbar: React.FC = () => {
  ...
  // Config progressColor overrides the Mantine primary color when set
  const safeThemeColor = config.theme.progressColor
    || theme.colors?.[theme.primaryColor]?.[theme.fn?.primaryShade() ?? 8]
    || '#ef4444';

  const { classes, cx } = useStyles({ glass, safeThemeColor });

  // Position from config: bottom (default), top, center
  const progressPos = config.theme.progressPosition ?? 'bottom';
  const wrapperPosStyle: React.CSSProperties =
    progressPos === 'top'    ? { bottom: 'auto', top: 0, paddingTop: '20px', paddingBottom: '50px', alignItems: 'flex-start' } :
    progressPos === 'center' ? { bottom: 'auto', top: '50%', transform: 'translateY(-50%)' } :
    {}; // default: bottom (handled by createStyles)
  ...
  return (
    <>
      <Box className={classes.wrapper} style={wrapperPosStyle}>
