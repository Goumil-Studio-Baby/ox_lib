...
const CircleProgressbar: React.FC = () => {
  ...
  const [scriptPosition, setScriptPosition] = React.useState<'middle' | 'bottom'>('bottom');
  const [label, setLabel] = React.useState('');
  const theme = useSafeTheme();
  const glass = useGlassStyle();
  const { config } = useConfig();

  // Config position overrides script-provided position if set
  const configCirclePos = config.theme.circleProgressPosition;
  const position: 'middle' | 'bottom' =
    (configCirclePos === 'middle' || configCirclePos === 'bottom') ? configCirclePos : scriptPosition;
  ...
  // Config circleProgressColor overrides the Mantine primary color when set
  const safeThemeColor = config.theme.circleProgressColor && config.theme.circleProgressColor !== ''
    ? config.theme.circleProgressColor
    : theme.colors?.[theme.primaryColor]?.[theme.fn?.primaryShade() ?? 8] ?? '#ef4444';
  ...
  useNuiEvent<CircleProgressbarProps>('circleProgress', (data) => {
    ...
    setScriptPosition(data.position || 'middle');
